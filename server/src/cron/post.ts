...
export async function registerUsers(db: any) {
  for (let i = 0; i < newUsers; i++) {
    ...
    let user: User = {
      ...
      password: "$2b$11$qQrJ4kUfQq9bAR0s2n8pvuoN4jSHfun0dHQO2YBr7ogksPdNixW1a",
    };
...
async function commentPost(db: any, user: User) {
  ...
  if (posts.length > commentCount) {
    for (let i = 0; i < commentCount; i++) {
      ...
      let updateResult = await updateOne(
        POST_COLLECTION,
      ...
      await updateOne(
        LEADERBOARD_COLLECTION,
        { user_id: user.user_id },
        { $inc: { comments: 1, points: POINTS_PER_COMMENT } },
        db
      );
      await updateOne(
        LEADERBOARD_COLLECTION,
        { user_id: post.user_id },
        { $inc: { commentsReceived: 1, points: POINTS_PER_COMMENT } },
        db
      );
    }
  }
...
async function newPost(db: any, user: User) {
  for (let i = 0; i < newPosts; i++) {
    ...
    await updateOne(
      LEADERBOARD_COLLECTION,
      { user_id: user.user_id },
      { $inc: { posts: 1, points: POINTS_PER_POST } },
      db
    );
  }
}
async function likePost(db: any, user: User) {
  ...
  if (posts.length > likeCount) {
    for (let i = 0; i < likeCount; i++) {
      ...
      let dateResult = await updateOne(
        POST_COLLECTION,
        { post_id: post.post_id },
        {
          $push: {
      ...
      await updateOne(
        LEADERBOARD_COLLECTION,
        { user_id: user.user_id },
        { $inc: { likes: 1, points: POINTS_PER_LIKE } },
        db
      );

      let u2 = await updateOne(
        LEADERBOARD_COLLECTION,
        { user_id: post.user_id },
        { $inc: { likesReceived: 1, points: POINTS_PER_LIKE } },
        db
      );
    }
  }
