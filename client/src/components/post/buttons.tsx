...
export const LikeButton = ({ likes, postPubkey }: LikeButtonProps) => {
  ...
  async function likePost0() {
      let response = "";
      if (didLike) {
        // response = await unlike({ wallet: programContext?.getWallet, program: programContext?.postProgram, postPubkey });
      } else {
        // response = await like({ wallet: programContext?.getWallet, program: programContext?.postProgram, postPubkey });
      }
      if (response === "liked") {
        setDidLike(true);
